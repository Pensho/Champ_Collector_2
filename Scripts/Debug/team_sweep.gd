			...
			for k in range(j + 1, p_presets.size()):
				...
				if(null != best):
					rows.append({
						...
						"crit_chance": best.crit_chance,
						"crit_damage_multiplier": best.crit_damage_multiplier,
						"crit_factor": best.crit_factor,
						"burst_crit_factor": best.burst_crit_factor,

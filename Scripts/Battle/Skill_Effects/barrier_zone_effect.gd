...
## It reads zone_damage_multiplier — not a Barrier's own scaling, but the scalar a repeat
## resolution of the whole zone carries (ZoneResolver.ResolveZoneEffectEcho), which the
## Barrier owes the same as a damage zone owes its own.
...
func Resolve(p_context: SkillCastContext) -> void:
	for target_ID in p_context.TargetsFor(self):
		Skills.ApplyBarrierZone(p_context.resolver, p_context.caster_ID, p_context.zone_ID,
				...
				p_context.zone_damage_multiplier)

...
| `ContentPool` | `Scripts/Battle/content_pool.gd` | Build content allowlist: the reagent families and Relic keys random draws (loot, shop) may produce, the champions `FortuneFavorTier.RecruitableChampions()` may offer, and the starting roster. `Active()` resolves the mode: a test pin (`PinForTest`) overrides everything else; otherwise playtest is active when the `CHAMP_COLLECTOR_BUILD_MODE` environment variable equals `playtest` or the `playtest` feature tag is present, and unrestricted otherwise. The environment variable can only select playtest, never suppress the feature tag. Key lookups, save loading and debug tools always read the full registries |

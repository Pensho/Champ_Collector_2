...
| `ContentPool` | `Scripts/Battle/content_pool.gd` | One game mode's content, as a resource under `Data/Game_Modes/`: its `mode_name`, whether it `is_default_mode`, the reagent families and Relic keys random draws (loot, shop) may produce, the champions `FortuneFavorTier.RecruitableChampions()` may offer, the starting roster, and `starting_reagent_count` for `RollStartingReagentKeys()`. The full game is one of these, not the absence of one. Key lookups, save loading and debug tools always read the full registries |
| `GameModeRegistry` | `Scripts/Battle/game_mode_registry.gd` | Selects the mode this run obeys. `ResolveModeName()` takes the first of two sources that names one: the `CHAMP_COLLECTOR_BUILD_MODE` environment variable, then the `champ_collector/build/content_pool_mode` project setting (Project Settings -> Champ Collector -> Build, the editor play-run switch, and what an exported build carries). Both unset runs the default mode, and an unrecognized name warns and does the same. Selection reads only these two, never `OS.has_feature()`, so a mode is never chosen by anything the project does not state as text. `Active()` returns that mode's pool and never returns null, with a test pin (`PinForTest`) overriding everything. `POOLS` preloads every mode, so adding one is its resource plus a line there. `MainInstance` prints the resolved mode at boot |

...
- **Split Relic from the rarity enum** *(Priority: High | Effort: M)*
  `Types.Rarity.Relic = 6` makes Relic a rarity value, while `Concept_Document.md` 3.3.1 defines it as an item type orthogonal to rarity. A Relic therefore has no rarity ladder: `EquipmentPreset.Setup()` skips its attribute roll entirely, `Equipment.GetUpgradeGain()` returns 9 (above Legendary's 8), and `Equipment.Upgrade()` scatters that across the whole slot pool for 90 points against a Legendary standard item's 105 — instead of 3.3.1's half. Every five-step ladder in `Relic_Design.md` needs the split before any of it is implementable, Two entries additionally need hooks `character_trait.gd` does not define: Sunderplate Nail a `GetOutgoingDefenceIgnoreFactor`, and Laden Coffer a `GetRewardMultiplier` plus a third argument on `LootManager.DistributeRewards()` to carry it. The effect mechanism further needs character-trait dispatch routed through a collection instead of the single `_trait` field it reads across roughly forty call sites in `Scripts/Battle/`, so a character's trait and their equipped Relics both dispatch; status instances carrying their applier, and ally-event hooks reaching the wearer's Relic, neither of which is wired; and `Skills.AppliedAttributeAmplification()` summing across the team rather than taking the max, so a Relic's amplification adds to a passive's instead of being discarded by it.

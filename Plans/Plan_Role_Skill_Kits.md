...
Not started. Independent of the implementation plans; can run at any time. One overlap:
...
- **Herald of the loom:** decide during its batch where the three thread stances live
  (a stance-switching skill vs. the passive slot) before writing the kit.
...
Weight of Law, Pagan Curse, Zap, Heap on, Stab. Whatever remains unclaimed stays in
3.2.4.3.
...
- Passive synergy: a kit that ignores its Role's passive economy (e.g. a Sorcerer kit
  of all basic skills, starving Arcane Instability) fails the theme requirement even
  if it violates no overlap rule.

...
##
## Runs Scripts/Debug/blowout_calibration.gd's five reports through the same GUT entry path
...
## that graph reachable from blowout_calibration.gd. The five _Report* methods are static on
## BlowoutCalibration for exactly this reason: this file calls them directly rather than
## instantiating a SceneTree.

func test_prints_all_five_calibration_reports() -> void:
	...
	assert_true(true, "Five calibration reports printed above")

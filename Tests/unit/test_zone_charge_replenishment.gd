...
func _place_zone(p_duration: int) -> void:
...
func test_replenish_raises_duration_up_to_the_cap() -> void:
	...
	assert_eq(_resolver.GetZoneResolver().GetZones()[0]._duration, 3)
...
func test_replenish_emits_zone_duration_changed() -> void:
	...
	var changes: Array[CombatResult] = _resolver._batch.filter(
			func(r): return r.kind == CombatResult.Kind.Zone_Duration_Changed)
	assert_eq(changes.size(), 1)
	assert_eq(changes[0].zone_ID, 0)
	assert_eq(changes[0].duration, 3)
...
func test_replenish_is_capped_and_does_not_exceed_max_charges() -> void:
	...
	assert_eq(_resolver.GetZoneResolver().GetZones()[0]._duration, 5)
...
func test_replenish_is_a_no_op_at_the_cap() -> void:
	...
	var changes: Array[CombatResult] = _resolver._batch.filter(
			func(r): return r.kind == CombatResult.Kind.Zone_Duration_Changed)
	assert_true(changes.is_empty(), "No-op at the cap should not emit Zone_Duration_Changed")
...
func test_replenish_is_a_no_op_above_the_cap() -> void:
	...
	assert_eq(_resolver.GetZoneResolver().GetZones()[0]._duration, 7)
	var changes: Array[CombatResult] = _resolver._batch.filter(
			func(r): return r.kind == CombatResult.Kind.Zone_Duration_Changed)
	assert_true(changes.is_empty(), "No-op above the cap should not emit Zone_Duration_Changed")

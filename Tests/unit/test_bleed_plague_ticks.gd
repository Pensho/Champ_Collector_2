...
# ticks. Plague is also stackable (Concept_Document.md 3.2.3.2) — multiple applications
# tick independently rather than merging.
...
func test_plague_stacks_independently_instead_of_refreshing() -> void:
	_apply_debuff(Types.Debuff_Type.Plague, 0, 1, 2)
	_apply_debuff(Types.Debuff_Type.Plague, 0, 3, 3)

	assert_eq(_roster[0]._active_debuffs.size(), 2, "A stackable debuff should append a new instance, not merge")

...
func test_every_implemented_buff_type_has_registry_data() -> void:
	for buff_name in Types.Buff_Type.keys():
		...
		assert_not_null(data, "Buff_Type.%s has no StatusEffectData registered" % buff_name)
		if(null != data):
			assert_not_null(data.icon, "Buff_Type.%s's StatusEffectData has no icon" % buff_name)
...
func test_every_implemented_debuff_type_has_registry_data() -> void:
	for debuff_name in Types.Debuff_Type.keys():
		...
		assert_not_null(data, "Debuff_Type.%s has no StatusEffectData registered" % debuff_name)
		if(null != data):
			assert_not_null(data.icon, "Debuff_Type.%s's StatusEffectData has no icon" % debuff_name)
